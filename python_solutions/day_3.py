...
'''
### Parse input file into list of lists for each row in input data
- (i.e. each nested list = row from input data)
- Replace tree with 1 & open space with 0 (as integers)
    - Want to count number of trees, so with if tree is 1 can do a simple sum
'''
...
After a few rows, would reach end of provided input - need to copy rows (pattern repeats) so can reach bottom of map

- Would be cool to try to do miminum extension required for each list
...
After 10 rows, will have to add another duplicate of pattern

if have index (row, column) starting at (0,0)
(1,3)
(2,6)

(9,30) is last row in which column index can fit before needing to copy the pattern
...
e.g. workflow would be
nested = [[1,2], [3,4]]
n_duplicate = [1,2] #don't copy [1,2], copy [3,4] once

def duplicate_pattern(row_list, n):
    return row_list * n

list(map(duplicate_pattern, nested, n_duplicate))
[[1, 2, 3], [4, 5, 6, 4, 5, 6]]

To programatically get n_duplicate, I need to know
- starting index/position for row & column
...
To work out how many times can copy a pattern n times in a range of rows,
and column index still remain inside range of list

e.g. Pattern of length 31 when jump 1 row, 3 columns at a time, starting at (0,0) (top left)
= (length of row) - 1 / col_start_index + column_jump
= 31 - 1 / 0 + 3
= 10

For rows that don't want to copy pattern, need to assign index for that row in list to 1
...
Initalise index list with first n rows that don't want to copy

row_index_start + number_col_jumps_per_copy
i.e. this is the number of rows that don't want to copy (assign value of 1)

index_list = [1] * row_jump * (init_row_index + n_col_jumps_per_copy)
...
Now I'm bored, let's just code it...
'''
...
def get_n_copies_list(init_row_index = 0, init_col_index = 0, row_jump = 1, col_jump = 3, row_length = 31, list_length = 323):
    ...
    while len(copies_list) <= list_length:
        ...
        idx_sequence.extend(rows_to_skip) # if rows to skip empty, idx_sequence [n_copies]
    ...
    #While loop means that copies_list could end up having indexes for rows not in data set (be better Sam...)
    ...
    if len(copies_list) == list_length:
        ...
        if len(copies_list) == list_length:
            ...
            raise Exception("You've messed up somewhere with the number of copies index...")



pt1_row_n_copies_list = get_n_copies_list()
...
def duplicate_pattern(row_list, n):
    '''
    return list with contents copied in order n times
    e.g.
    x = [1,2]
    duplicate_pattern(x,2)
    [1,2,1,2]
    '''
    return row_list * n
...
cp_nested_data = list(map(duplicate_pattern, nested_data, pt1_row_n_copies_list))
...
'''
Now have copied pattern matrix/list of list

Need to count how many trees do you come across when go 1 down, 3 across

Stopping when get to final row
